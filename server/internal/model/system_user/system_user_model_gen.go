...
func (m *defaultSystemUserModel) Update(ctx context.Context, data *SystemUser) error {
	...
	sql, _ := sb.Build()
	_, err := m.conn.ExecCtx(ctx, sql, data.CreateTime, data.UpdateTime, data.CreateBy, data.UpdateBy, data.Username, data.Password, data.Nickname, data.Gender, data.Phone, data.Email, data.Status, data.Id)
	return err
}
