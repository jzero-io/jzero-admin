...
var (
	...
	cacheSystemMenuIdPrefix = "jzero-admin:cache:systemMenu:id:"
)
...
func (m *defaultSystemMenuModel) DeleteWithCache(ctx context.Context, session sqlx.Session, id uint64) error {
	systemMenuIdKey := fmt.Sprintf("%s%v", cacheSystemMenuIdPrefix, id)
...
func (m *defaultSystemMenuModel) FindOneWithCache(ctx context.Context, session sqlx.Session, id uint64) (*SystemMenu, error) {
	systemMenuIdKey := fmt.Sprintf("%s%v", cacheSystemMenuIdPrefix, id)
	var resp SystemMenu
	err := m.cachedConn.QueryRowCtx(ctx, &resp, systemMenuIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
		sb := sqlbuilder.Select(systemMenuRows).From(m.table)
		sb.Where(sb.EQ("`id`", id))
		sql, args := sb.Build()
		if session != nil {
...
func (m *defaultSystemMenuModel) InsertWithCache(ctx context.Context, session sqlx.Session, data *SystemMenu) (sql.Result, error) {
	systemMenuIdKey := fmt.Sprintf("%s%v", cacheSystemMenuIdPrefix, data.Id)
	statement, args := sqlbuilder.NewInsertBuilder().
		InsertInto(m.table).
		Cols(systemMenuRowsExpectAutoSet).
		Values(data.CreateTime, data.UpdateTime, data.CreateBy, data.UpdateBy, data.Status, data.ParentId, data.MenuType, data.MenuName, data.HideInMenu, data.ActiveMenu, data.Order, data.RouteName, data.RoutePath, data.Component, data.Icon, data.IconType, data.I18nKey, data.KeepAlive, data.Href, data.MultiTab, data.FixedIndexInTab, data.Query, data.Buttons, data.Constant).Build()
...
func (m *defaultSystemMenuModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheSystemMenuIdPrefix, primary)
}

func (m *defaultSystemMenuModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	sb := sqlbuilder.Select(systemMenuRows).From(m.table)
