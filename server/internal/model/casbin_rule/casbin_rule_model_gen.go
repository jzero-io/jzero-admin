...
var (
	...
	cacheCasbinRuleIdPrefix = "jzero-admin:cache:casbinRule:id:"
)
...
func (m *defaultCasbinRuleModel) FindOneWithCache(ctx context.Context, session sqlx.Session, id int64) (*CasbinRule, error) {
	casbinRuleIdKey := fmt.Sprintf("%s%v", cacheCasbinRuleIdPrefix, id)
	var resp CasbinRule
	err := m.cachedConn.QueryRowCtx(ctx, &resp, casbinRuleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
		sb := sqlbuilder.Select(casbinRuleRows).From(m.table)
		sb.Where(sb.EQ("`id`", id))
		sql, args := sb.Build()
		if session != nil {
